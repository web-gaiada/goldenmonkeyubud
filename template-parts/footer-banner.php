<?php if (have_rows('jumbotron')): ?>
            ...
            <div class="position-absolute top-50 start-50 translate-middle text-white text-center white-link">
                ...
                <?php if (have_rows('section_button')): ?>
                    <?php while (have_rows('section_button')):
                        the_row(); ?>
                        <a href="<?php bloginfo('url') ?><?php the_sub_field('button_link'); ?>" class="btn btn-reserve fw-bold"
                            aria-label="Order now"><?php the_sub_field('button_text'); ?></a>
                    <?php endwhile; ?>
                <?php endif; ?>
            </div>
    ...
    <?php endwhile; ?>
<?php else: ?>
    <?php while (have_rows('jumbotron', 263)):
        the_row(); ?>
        <div class="position-relative h-100-desktop">
